...
export const GlobalStyles = createGlobalStyle`
  * {
    ${tw`text-black m-0 p-0 box-sizing[border-box] border[0 solid] font-sans`};
  }
`;
export const Layout = tw.div`width[100vw] height[100vh] flex justify-center items-center background-color[#93c5fd]`;
...
export const Footer = styled.div(({ smaller }) => [tw`text-gray-600 font-size[.9rem] font-light mb-2`, smaller && tw`font-size[.8rem]`]);
export const Link = tw.a`text-decoration[none]`;
export const SpanBold = tw.span`font-bold`
export const Container = tw.div`relative flex flex-col items-center border-width[10px] border-blue-400 rounded-lg box-shadow[0 10px 10px rgba(0,0,0,.2)] mb-5`;
...
export const Player1Bar = tw.div`bg-red-600 w-full h-12 rounded-l-full relative overflow-hidden border-width[3px] border-right-width[0] border-white`;
export const Player1Inner = styled.div(({ health }) => [
...
export const Player2Bar = tw.div`bg-red-600 w-full h-12 rounded-r-full relative overflow-hidden border-width[3px] border-left-width[0] border-white`;
export const Player2Inner = styled.div(({ health }) => [
...
export const Counter = tw.div`flex justify-center items-center bg-blue-300 w-16 h-16 rounded-full font-size[2.2rem] font-bold text-gray-800`;
