...
class Guide {
    ...
    draw() {
        ...
        c.fillText("遇到荆棘会扣除5~10分间的随机分数",start_pos.x+500,start_pos.y+130)
        c.drawImage(createImage('./img/clock2.png'), start_pos.x+460, start_pos.y+140)
        c.fillText("收集时钟可以增加5s时间",start_pos.x+500,start_pos.y+170)
        c.drawImage(createImage('./img/bomb.png'), start_pos.x+460, start_pos.y+180)
        c.fillText("踩到炸弹游戏结束",start_pos.x+500,start_pos.y+210)
        c.fillText("通过w,s,a,d控制，当吃完全部豆子你将获得胜利！",start_pos.x+470,start_pos.y+250)
    }
}
...
function isCollide({circle,rectangle}) {
    return (
        circle.position.y - circle.radius <= start_pos.y
        || circle.position.x + circle.radius >= start_pos.x + size[1] * rectangle.width
        || circle.position.y + circle.radius >= start_pos.y + size[0] * rectangle.height
        || circle.position.x - circle.radius <= start_pos.x
    )
}
